...
//				Pattern p = Pattern.compile("\\/\\/\\s*sync\\s+(.+\\s*,\\s*\\w+)");
				// for ruby
				// the below without #endSync, as what we have in java 
				Pattern p = Pattern.compile("#\\s*sync\\s*(.+\\s*,\\s*\\w+)");
//				Pattern p = Pattern.compile("#\\s*sync\\s*(.+?),\\s*([^\\s]+)\\s*([\\w\\W]+?)#\\s*endSync");
			...
			} else if (line.contains("=begin sync")) {
...
//					Pattern p = Pattern.compile("\\s(.+\\s*,\\s*\\w+)\\s*\\*\\/\\s(\\w+)\\s*\\/\\*\\s*endSync\\s*\\*\\/\\s+\\w+\\s+\\/\\*\\s*sync\\s+(.+\\s*,\\s*\\w+)\\s*\\*\\/\\s*(\\w+)\\s*\\/\\*\\s*endSync\\s*\\*\\/");
					// for ruby
					Pattern p = Pattern.compile("=begin sync\\s*(.+?),\\s*([^\\s]+)\\s*(?:=end|\\n)([\\w\\W]+?)(?:=begin\\s*endSync\\s*=end)");
...
//					final String regex = "\\/\\*\\s*sync\\s+(.+)\\s*,\\s*(\\w+)\\s*\\*\\/\\s*(\\w+)\\s*\\/\\*\\s*endSync\\s*\\*\\/";
					// for ruby, But not sure if still need it or not
					final String regex = "=begin\\s*sync\\s*\\s+(.+)\\s*,\\s*(\\w+)\\s*=end\\s* (\\w+)\\s*=begin\\s*endSync\\s*=end"; // correct
